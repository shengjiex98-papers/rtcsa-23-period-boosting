...
# ╔═╡ 0caaf962-b376-410c-a5c3-853eb335faa6
begin
	using Plots
...
# ╔═╡ 909463ac-419d-11ec-36d5-110f3cc4155f
md"# Experiments for extension"

# ╔═╡ 6079c28e-d701-4e15-bc84-b1c47e0faf5a
@bind ctrl_delay NumberField(0:0.05:1, default = 0.1)

# ╔═╡ eea03ed4-738f-4889-b710-ce2e973689c3
begin
	sys = let
		A = [-1 1; 0.2 0.2]
		B = [0; -2]
	...
	sysd_delayed = let
		...
		C = [1 0 0]
		D = 0
		ss(Φ, Γ, C, D, ctrl_delay)
	end
	
	K = dlqr(sysd_delayed, I, I)
	sysd_closed = let
		Φ = (sysd_delayed.A - sysd_delayed.B * K)
		Γ = zeros(3, 1)
		C = [1 0 0]
		D = 0
		ss(Φ, Γ, C, D, ctrl_delay)
...
# ╔═╡ 3d24fcae-0813-4623-977d-78851cf3a5ed
begin
	y, t, x, u = lsim(sysd_delayed, (x, t) -> -K*x, 0:ctrl_delay:5, x0=[1; 1; 0])
	p = plot(t, x'[:,1:2])
end

# ╔═╡ d8978235-2a11-4d51-8219-e0e2b8642346
begin
	x_comp = plot(layout = (2, 1), label = ["x1", "x2"])
	for delay in 0.05:0.05:0.5
		sysd = c2d(sys, delay)
		sysd_delayed = let
			Φ = [sysd.A sysd.B;
				 zeros(1, 3)]
		...
		y, t, x, u = lsim(sysd_delayed, (x, t) -> -K*x, 0:delay:5, x0=[1; 1; 0])
		plot!(x_comp[1], t, x'[:, 1], label = delay)
		plot!(x_comp[2], t, x'[:, 2], label = delay)
	end
	x_comp
end
