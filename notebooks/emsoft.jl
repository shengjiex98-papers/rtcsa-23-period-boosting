...
# ╔═╡ b47583ef-0f3e-4e84-a480-694ddd60595f
function HoldAndSkipNextSlide(sysd, K, miss=nothing, window=1)
	p, r = size(sysd.B)
...
# ╔═╡ 3bf69191-f3e4-45c8-91cd-f75a890ec302
function HoldAndSkipNextCons(sysd, K, miss=nothing)
	p, r = size(sysd.B)
	...
	elseif miss == 0
		...
		μ = zeros(Union{Missing, Int64}, (L, 2))
		for i = 0:L-1
			# i represents current number of misses
			T[i+1, 1] = (i + 1) + 1		# miss
			T[i+1, 2] = 0 + 1			# hit
			μ[i+1, 1] = 4 - (i == 0)
			μ[i+1, 2] = 2 - (i == 0)
...
begin
	strat_map = Dict(
		...
		"Hold&Skip-Next-Slide" => HoldAndSkipNextSlide#,
...
# sysd = let
# 	# ======== Electric Steering ========
# 	# A = [0.996 0.075; -0.052 0.996]
# 	# B = [0.100 0.003; -0.003 0.083]
# 	# C = [0 0]
# 	# D = [0 0]
...
# ╔═╡ 140fdac4-e299-40c4-aa64-948e21afb414
function compute_k(sysd_delay, state_cost, control_cost)
	Q = state_cost * I
	R = control_cost * I
	print(Q, R)
	dlqr(sysd_delay, Q, R)
...
# K = compute_k(sysd_delay, state_cost, control_cost)

# ======== Electric Steering ========
K = [0.9067 0.07384 0 0; 0.01041 0.9685 0 0]
...
# ╔═╡ e15297b6-b07f-4a7f-a21a-4714892ce096
print("$(HoldAndSkipNextSlide(sysd_delay, K, 1, 4).T)")

# ╔═╡ ab1ebca5-c3f8-4bc2-8bd2-a8dde50c5a8b
print("$(HoldAndSkipNextCons(sysd_delay, K, 2).μ)")
...
let
	...
	print(sysd, K)
	hsn = HoldAndSkipNextSlide(sysd, K)
	...
	for i = size(hit_prob, 1):-1:1
		...
		plot!(0:sim_time, z[:,3], label=L"P(\mathrm{hit}) = %$(hit_prob[i])", linecolor=line_colors[i])
	end
	plot!()
end
...
begin
	points_one_bounded_tree = let
		automaton = strat_map[sim_strategy_one_bounded_tree](sysd, K, maxmiss_one_bounded_tree, 8)
...
|                 $n$ | $(@bind n_trees Slider(1:20, default=5, show_value=true))    |
...
# ╔═╡ d5f70e40-6af8-43fa-bfe6-68007d580658
md"""
## Synthesizing constraints for a given deviation bound"""

# ╔═╡ a0b1a335-df1e-4f39-a0e1-fe0a667ab419
md"""
Adjust the number of steps per tree $n$, and the total number of time steps $t$ for synthesizing the constraints.

|                     |                                                              |
|--------------------:|:-------------------------------------------------------------|
|                 $n$ | $(@bind n_syn Slider(1:20, default=5, show_value=true))    |
|                 $t$ | $(@bind time_syn Slider(5:5:500, default=100, show_value=true)) |
"""

# ╔═╡ 89f7bcd6-46c5-4c07-95de-889390081f1d
md"""
The following table shows the max deviation for the weakly-hard constraint
$$\overline{\begin{pmatrix}
    n \\
    m \\
\end{pmatrix}}$$
with $m$ (window size) on the left and $n$ (missed deadlines) on top.
"""

# ╔═╡ 81815aad-2b09-49f3-ae7e-fa45b888ddc7
md"""
The following table shows the max deviation for the weakly-hard constraint
$$\overline{\langle n \rangle}$$
where $n$ is the maximum number of consecutive deadline misses.
"""
...
begin
	...
	all_bounds = let
		automaton = if sim_strategy_trees === "Hold&Skip-Next-Slide"
			HoldAndSkipNextSlide(sysd, K, max_miss_trees, window_size_trees)
		else
			strat_map[sim_strategy_trees](sysd, K, max_miss_trees)
		end
		# automaton = HoldAndSkipNextSlide(sysd, K, max_miss_trees, window_size_trees)
		augbounds = Augment(bounds, automaton)
...
let
	...
	if v < 1e3
		...
		#plot!([0, 75, 150], [1.80, 1.80, 1.80], label="", color=:black, series_annotations=["", text("Safety bound", :bottom), ""])
	else
...
# ╔═╡ 34858360-8027-4f72-ad12-0ddcc3fd16d1
function synthesize(max_window_size=10)
	# Printing section. Change this later
	md_buffer = IOBuffer()

	# Printing # of misses
	print(md_buffer, "| ")
	for window_size in 1:max_window_size-1
		print(md_buffer, "| $window_size ")
	end
	print(md_buffer, "|\n")

	# Printing the separater line
	print(md_buffer, "|--:")
	for _ in 1:max_window_size-1
		print(md_buffer, "|--:")
	end
	print(md_buffer, "|\n")
	# ====================================
	...
	for window_size in 2:max_window_size
		...
		for max_misses in 1:(window_size-1)
			automaton = HoldAndSkipNextSlide(sysd, K, max_misses, window_size)
			augbounds = Augment(bounds, automaton)
			all_bounds = BoundedTreeIter(automaton, augbounds, n_syn, time_syn)
			d = deviation(automaton, augbounds, all_bounds, dims=[1,2])
			i = argmax(d)
			v = maximum(d)

			# Printing
			@printf(md_buffer, "| %.3f ", v)
			# ========
		end

		for _ in window_size:max_window_size-1
			print(md_buffer, "| ")
		end
		# Printing
		print(md_buffer, "| \n")
	...
	# Return the markdown
	Markdown.parse(String(take!(md_buffer)))
end

# ╔═╡ 20336440-810f-4655-906a-b57246c3261e
# ╠═╡ disabled = true
#=╠═╡
synthesize(5)
  ╠═╡ =#

# ╔═╡ 2239da4b-0f26-4a8c-a946-54c9c174ee10
function synthesizeCons(max_misses)
	# Printing section. Change this later
	md_buffer = IOBuffer()

	# Printing # of misses
	for cons_misses in 1:max_misses
		print(md_buffer, "| $cons_misses ")
	end
	print(md_buffer, "|\n")
	...
	for cons_misses in 1:max_misses
		automaton = HoldAndSkipNextCons(sysd, K, cons_misses)
		augbounds = Augment(bounds, automaton)
		all_bounds = BoundedTreeIter(automaton, augbounds, n_syn, time_syn)
		d = deviation(automaton, augbounds, all_bounds, dims=[1,2])
		i = argmax(d)
...
# ╔═╡ 8ff8bb92-96b9-4da7-85c2-6bc6ccdf585b
# ╠═╡ disabled = true
#=╠═╡
synthesizeCons(2)
  ╠═╡ =#
